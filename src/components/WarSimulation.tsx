...
export default function WarSimulation({ participants, playerServant, onClose }: Props) {
  ...
  const downloadShareCard = useCallback(async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(shareCardRef.current, { backgroundColor: "#0a0a1a", scale: 2 });
      ...
      const text = playerWon
        ? `성배전쟁 승리! ${playerServant.name}(${playerServant.class})가 ${rounds.length}일차에 성배를 획득. 소원: "${wish}"`
        : `성배전쟁 패배. ${playerServant.name}(${playerServant.class})가 ${playerLostDay ?? "?"}일차에 탈락.`;
      navigator.clipboard.writeText(text);
      alert("이미지 생성 실패 — 텍스트가 클립보드에 복사되었습니다.");
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-4" style={{ paddingTop: "2rem" }}>
                ...
                {latestRound.isQuiet ? (
                  ...
                  <div className="space-y-3">
                    {latestRound.battles.map((b, i) => {
                      const aPct = Math.round(b.result.winProbability * 100);
                      const bPct = 100 - aPct;
                      return (
                        <div
                          key={i}
                          ...
                          {b.result.skillEffects.length > 0 && (
                            <div className="mb-2 space-y-0.5">
                              {b.result.skillEffects.map((effect, j) => (
                                <p key={j} className="text-xs text-magic-blue text-center">
                                  {effect.description}
                                </p>
                              ))}
                            </div>
      ...
      {/* War Log (all rounds) - expanded by default */}
      {rounds.length > 1 && (
        <div className="w-full max-w-xl mb-6">
          <p className="text-sm text-gray-400 font-bold mb-3 text-center">전쟁 기록 ({rounds.length}일차)</p>
          <div className="space-y-2 max-h-80 overflow-y-auto">
            {rounds.map((round) => (
              <div key={round.day} className="text-sm text-gray-400 p-3 rounded-lg bg-white/3" style={{ background: "rgba(255,255,255,0.02)" }}>
                <span className="text-gold font-bold">제{round.day}일차:</span>{" "}
                {round.isQuiet
                  ? <span className="text-gray-600">아무 일도 일어나지 않았다...</span>
                  : round.battles.map((b, i) => (
                    <span key={i}>
                      {i > 0 && " / "}
                      {b.result.description}
                      {b.result.skillEffects.length > 0 && (
                        <span className="text-magic-blue"> [{b.result.skillEffects.map((e) => e.description).join(", ")}]</span>
                      )}
                    </span>
                  ))}
              </div>
        ...
        >
          {playerWon && !wishSubmitted ? (
            <div className="rounded-xl p-6 text-center relative overflow-hidden" style={{ background: "#0d0d24", border: "1px solid #ffd70044" }}>
              ...
              />
              <img src="/7999.png" alt="Holy Grail" className="w-20 h-20 mx-auto mb-3 object-contain animate-pulse-glow relative z-10" />
            ...
            >
              ...
              <div className="rounded-xl p-6 text-center relative overflow-hidden" style={{ background: "#0d0d24", border: "1px solid #ffd70044" }}>
                ...
                <div className="mt-4 p-3 rounded-lg bg-gold/5 border border-gold/20 relative z-10">
              ...
              <div
                ...
                style={{ background: "#0a0a1a", border: "2px solid #ffd700" }}
              >
                <div className="flex items-center justify-center gap-3 mb-3">
                  <ServantFace servant={playerServant} size={12} />
                  <div className="text-left">
                    <p className="text-lg font-bold text-gold" style={{ fontFamily: "var(--font-serif)" }}>성배전쟁 승리</p>
                    <p className="text-sm text-gray-300">{playerServant.name} ({playerServant.class})</p>
                  </div>
                </div>
                <p className="text-xs text-gray-500 mb-2">{rounds.length}일차에 성배 획득</p>
                <div className="p-2 rounded bg-gold/10 border border-gold/20">
                  <p className="text-[10px] text-gray-500">소원</p>
                  <p className="text-gold text-sm font-bold">"{wish}"</p>
                </div>
                <p className="text-[10px] text-gray-700 mt-3">성배전쟁 시뮬레이터 | Holy Grail War Simulator</p>
              </div>
            ...
            >
              ...
              {/* Share card for defeat */}
              <div
                ref={shareCardRef}
                className="rounded-xl p-6 text-center"
                style={{ background: "#0a0a1a", border: "2px solid #ff4a4a" }}
              >
                <div className="flex items-center justify-center gap-3 mb-3">
                  <ServantFace servant={playerServant} size={12} />
                  <div className="text-left">
                    <p className="text-lg font-bold text-magic-red" style={{ fontFamily: "var(--font-serif)" }}>성배전쟁 패배</p>
                    <p className="text-sm text-gray-300">{playerServant.name} ({playerServant.class})</p>
                  </div>
                </div>
                <p className="text-xs text-gray-500">{playerLostDay ?? "?"}일차에 탈락</p>
                {winner && <p className="text-xs text-gray-500 mt-1">승자: {winner.name} ({winner.class})</p>}
                <p className="text-[10px] text-gray-700 mt-3">성배전쟁 시뮬레이터 | Holy Grail War Simulator</p>
              </div>
